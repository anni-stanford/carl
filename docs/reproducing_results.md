...
pip install -e ".[dev]"
cp .env.example .env  # fill in ANTHROPIC_API_KEY (required) + optional OPENAI_API_KEY for judge rotation
docker compose -f docker/compose.yml up -d  # Postgres + dashboard

# Smoke test (no real episodes, just the loop wiring + tests)
pytest -q

# Reproduce E1 (main result) on FastAPI
bash scripts/run_all.sh --experiment e1 --repo fastapi
```
